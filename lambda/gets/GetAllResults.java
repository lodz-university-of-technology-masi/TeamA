...
public class GetAllResults implements RequestHandler<Void, ServerlessOutput>{
	...
	public ServerlessOutput handleRequest(Void input, Context context) {
        ...
        try {
           ...
           for (Map<String, AttributeValue> item : result.getItems()){
            	body += "{";
            	body += "\"resultId\":\"" + item.get("ResultId").getS() + "\",";
        	    body += "\"formTitle\":\"" + item.get("FormTitle").getS() + "\",";
        	    body += "\"hrEmployer\":\"" + item.get("HrEmployer").getS() + "\",";
        	    body += "\"owner\":\"" + item.get("Owner").getS() + "\",";
        	    body += "\"points\": [";
